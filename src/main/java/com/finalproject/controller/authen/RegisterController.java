		...
		if (userService.findByEmail(email) != null) {
			...
			if (userDTO.isVerify() == true) {
				...
				if (!password.equals(repassword)) {
					...
					userDTO.setAvatar("1608484153089.jpg");
					userService.update(userDTO);
					return "authen/register";
				} else {
					userDTO.setPassword(new BCryptPasswordEncoder().encode(password));
					userService.update(userDTO);
					sendEmail("pytun120@gmail.com", email, "Welcome to NTShop!",
							"Hello, " + email.split("@")[0] + "! Please confirm that you can login in NTShop!" + " Your confirmation code is: " + code);
				}
			}
		} else {
			if (!password.equals(repassword)) {
